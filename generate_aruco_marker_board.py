#!/usr/bin/env python3
"""Generate an ArUco marker-board calibration target."""
...
# 这里生成的是多个独立 ArUco marker 排列成的标记板，不是 ChArUco 棋盘。
...
# ArUco marker 排列数量。X 是横向 marker 数，Y 是纵向 marker 数。
MARKERS_X = 5
MARKERS_Y = 4

# 单个 ArUco marker 的实体边长。相机位姿估计时也要使用这个真实尺寸。
ARUCO_MARKER_MM = 30.0

# 相邻 marker 之间的白色间距。
MARKER_GAP_MM = 10.0

# 起始 marker id。脚本会按行优先顺序连续使用 id。
FIRST_MARKER_ID = 0
...
# 白色基板外形尺寸。
# 默认：内部 marker 区域 190 x 150 mm，基板 240 x 180 mm。
BASE_WIDTH_MM = 240.0
...
# rectangles_no_gaps：规则矩形模式，相邻黑色单元共享边不内缩，只在黑白交界处内缩。
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate ArUco marker-board PNG/SVG/DXF/STEP assets.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--output-dir", type=Path, default=OUTPUT_DIR)
    parser.add_argument("--output-prefix", default=OUTPUT_PREFIX)
    parser.add_argument("--markers-x", type=int, default=MARKERS_X)
    parser.add_argument("--markers-y", type=int, default=MARKERS_Y)
    parser.add_argument("--aruco-marker-mm", type=float, default=ARUCO_MARKER_MM)
    parser.add_argument("--marker-gap-mm", type=float, default=MARKER_GAP_MM)
    parser.add_argument("--first-marker-id", type=int, default=FIRST_MARKER_ID)
...
def main(argv: list[str] | None = None) -> int:
    ...
    args = core.make_args(
        board_type="aruco_marker_board",
        output_dir=parsed.output_dir,
        output_prefix=parsed.output_prefix,
        markers_x=parsed.markers_x,
        markers_y=parsed.markers_y,
        aruco_marker_mm=parsed.aruco_marker_mm,
        marker_gap_mm=parsed.marker_gap_mm,
        first_marker_id=parsed.first_marker_id,
        ...
        pixels_per_square=parsed.pixels_per_marker,
