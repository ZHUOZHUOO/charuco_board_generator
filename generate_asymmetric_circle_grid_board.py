#!/usr/bin/env python3
"""Generate an asymmetric circle-grid calibration board."""
...
# 非对称圆点采用 OpenCV 常见约定：
#   同一行相邻圆心距离 = 2 * CIRCLE_SPACING_MM
#   相邻行水平错开     = 1 * CIRCLE_SPACING_MM
#   相邻行垂直距离     = 1 * CIRCLE_SPACING_MM
...
# OpenCV asymmetric circle grid 的 spacing。相机标定时也要使用这个真实尺寸。
CIRCLE_SPACING_MM = 16.0

# 圆点直径。建议小于 CIRCLE_SPACING_MM，避免相邻行圆点过近。
CIRCLE_DIAMETER_MM = 8.0

# 白色基板外形尺寸。
# 默认：内部圆点区域 216 x 136 mm，基板 240 x 180 mm。
BASE_WIDTH_MM = 240.0
...
# STEP 黑色图案建模方式：
# rectangles_no_gaps：默认。规则几何模式，圆点会生成真实圆柱。
# contours_filtered：按图像轮廓内缩并过滤小岛/薄壁碎片，适合低面数 STEP。
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate asymmetric circle-grid calibration board PNG/SVG/DXF/STEP assets.",
...
def main(argv: list[str] | None = None) -> int:
    ...
    args = core.make_args(
        board_type="asymmetric_circle_grid",
